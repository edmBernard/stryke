...
TEST_CASE("OrcWriterSequentialDuplicate<TimestampNumber, ...> Split", "[Multifile]") {
  ...
  SECTION("Split by date") {
    ...
    SECTION("0 split") {
      {
        stryke::OrcWriterSequentialDuplicate<stryke::TimestampNumber, stryke::Int> writer({"date", "col1"}, root_folder, "test", options);
