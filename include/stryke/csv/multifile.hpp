...
class CsvWriterMulti {
public:
  CsvWriterMulti(std::array<std::string, sizeof...(Types) + 1> column_names, std::string root_folder, std::string file_prefix, const WriterOptions &options)
...
private:
    ...
    if (this->current_prefix_with_date.empty() || this->current_prefix_with_date != prefix_with_date || (this->writeroptions.nbr_batch_max > 0 && this->current_counts >= this->writeroptions.batchSize * this->writeroptions.nbr_batch_max)) {
      ...
      do {
        if (this->writeroptions.suffix_timestamp) {
          this->current_filename = file_folder / (this->current_prefix_with_date + "-" + std::to_string(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count()) + ".csv");
        } else {
          this->current_filename = file_folder / (this->current_prefix_with_date + "-" + std::to_string(this->current_suffix++) + ".csv");
        }
  ...
  void get_writer(T date) {
    if (get_name(date)) {
      ...
      this->writers = std::make_unique<CsvWriterImpl<T, Types...>>(this->column_names, this->current_filename, this->writeroptions);
      this->current_counts = 0;
    }
  }
  ...
  std::unique_ptr<CsvWriterImpl<T, Types...>> writers;
  int current_suffix;
